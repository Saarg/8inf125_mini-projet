...
void Raven_Game::ClickLeftMouseButton(POINTS p)
{
	...
	{
		static int shotCount = 0; //used to make sure we have about 50/50 data

		// Saves infos about current target
		double distance = MaxDouble;
		double angle = MaxDouble;
		Raven_Bot* aimedBot = 0;
		...
		{
			// If itself go to next bot
			if (*it == m_pSelectedBot)
				continue;

			// Calculate bot distance and angle
			double cur_distance;
			double cur_angle;
			...
			// If angle low enougth and lower than target angle
			if (cur_angle < 0.3 && abs(cur_angle) < abs(angle)) {
				// Save bot data as aimed bot
				aimedBot = *it;
		...
		// If there is a bot to aim train neural net
		if (aimedBot && shotCount <= 0) {
			fann_type calc_out[1];
			fann_type input[4];

			input[0] = (aimedBot)->Health();
			input[1] = distance;

			input[2] = m_pSelectedBot->GetWeaponSys()->GetCurrentWeapon()->GetIdealRange();
			input[3] = m_pSelectedBot->GetWeaponSys()->GetCurrentWeapon()->GetType();
		...
		for (it; it != bots.end(); ++it)
		{
			...
			if (cur_angle != angle && shotCount > 0) {
				// Train neural net not to shoot
				fann_type calc_out[1];
				fann_type input[4];
